...
void CombatEffectEditor::DrawLightning(LightningEffect& effect, const Vector3& origin,
	...
	ImGui::TextUnformatted("Short radial bolts for elemental hit impact");
	...
	ImGui::SliderInt("Main Bolt Count##Lightning", &lightning_.boltCount, 1, 16);
	ImGui::SliderFloat("Radial Spread##Lightning", &lightning_.spreadDegrees, 0.0f, 360.0f, "%.0f deg");
	...
	if (ImGui::Button("Thunder preset")) {
		...
		lightning_.jaggedness = 0.18f;
		lightning_.length = 1.8f;
		lightning_.width = 0.025f;
		lightning_.boltCount = 5;
		lightning_.branches = 1;
	}
	ImGui::SameLine();
	if (ImGui::Button("Dragon preset")) {
		lightning_.glowColor = { 0.45f, 0.01f, 0.06f, 0.8f };
		lightning_.coreColor = { 1.0f, 0.12f, 0.18f, 1.0f };
		lightning_.jaggedness = 0.3f;
		lightning_.length = 2.1f;
		lightning_.width = 0.035f;
		lightning_.boltCount = 7;
		lightning_.branches = 2;
	}
...
void CombatEffectEditor::DrawSlash(SlashEffect& effect, const Vector3& origin,
	bool previewRequested) {
	ImGui::TextUnformatted("Short crossing cut marks centered on the hit point");
	...
	ImGui::SliderFloat("Cut Curvature##Slash", &slash_.arcDegrees, 0.0f, 90.0f, "%.0f deg");
	...
	ImGui::SliderInt("Additional Cuts##Slash", &slash_.afterimages, 0, 8);
	ImGui::SliderFloat("Cut Angle Spacing##Slash", &slash_.afterimageSpacingDegrees, 0.0f, 120.0f, "%.1f deg");
