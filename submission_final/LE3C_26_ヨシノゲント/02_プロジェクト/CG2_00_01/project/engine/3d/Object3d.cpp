...
namespace {
	...
	constexpr float kMinimumAnimationTransitionSeconds = 1.0e-4f;

	float AdvanceAnimationTime(float time, float deltaTime, float playbackSpeed, float duration) {
		if (!std::isfinite(time) || !std::isfinite(playbackSpeed) || duration <= 0.0f) {
			return 0.0f;
		}
		time = std::fmod(time + deltaTime * playbackSpeed, duration);
...
void Object3d::Update(Camera* camera, float deltaTime) {
	...
	if (isAnimationPlaying_ && animation_.duration > 0.0f) {
		animationTime_ = AdvanceAnimationTime(
			animationTime_, deltaTime, animationSpeed_, animation_.duration);

		// --- フェーズ1：アニメーションの適用 ---
		// 手動コントロールフラグがオフのときのみアニメーションを骨に適用する
		if (skeleton_ && !isBoneManualControl_) {
			if (animationTransitionActive_) {
				transitionSourceTime_ = AdvanceAnimationTime(
					transitionSourceTime_, deltaTime, animationSpeed_, transitionSourceAnimation_.duration);
				animationTransitionElapsed_ = (std::min)(
					animationTransitionElapsed_ + deltaTime, animationTransitionDuration_);
				const float blendWeight = GetAnimationTransitionProgress();
				SkeletonSystem::ApplyAnimationBlend(
					*skeleton_,
					transitionSourceAnimation_,
					transitionSourceTime_,
					animation_,
					animationTime_,
					blendWeight);
				if (blendWeight >= 1.0f) {
...
void Object3d::SetAnimation(const Animation& animation) {
	...
	animationTransitionDuration_ = 0.0f;
}

bool Object3d::TransitionToAnimation(const Animation& animation, float durationSeconds) {
	if (!skeleton_ ||
		!std::isfinite(durationSeconds) ||
		durationSeconds < kMinimumAnimationTransitionSeconds ||
	...
	const float normalizedSourceTime = std::clamp(animationTime_ / animation_.duration, 0.0f, 1.0f);
	transitionSourceAnimation_ = animation_;
	transitionSourceTime_ = animationTime_;
	animation_ = animation;
	animationTime_ = std::fmod(normalizedSourceTime * animation_.duration, animation_.duration);
	animationTransitionElapsed_ = 0.0f;
	animationTransitionDuration_ = durationSeconds;
	animationTransitionActive_ = true;
	return true;
}

float Object3d::GetAnimationTransitionProgress() const {
	if (!animationTransitionActive_ || animationTransitionDuration_ < kMinimumAnimationTransitionSeconds) {
		return animationTransitionActive_ ? 0.0f : 1.0f;
	}
	return std::clamp(animationTransitionElapsed_ / animationTransitionDuration_, 0.0f, 1.0f);
}
