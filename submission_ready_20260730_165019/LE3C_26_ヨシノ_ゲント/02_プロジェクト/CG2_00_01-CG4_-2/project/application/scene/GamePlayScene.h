...
private:
	...
	void BindLeftHandParticleSocket();
	void EmitLeftHandWaterBurst();
	void DrawBusterSwordAttachmentGizmo() const;
	void DrawLeftHandParticleSocketGizmo() const;
...
private:
	...
	JointAttachmentSystem leftHandParticleSocket_;
	bool showBusterSwordGizmo_ = true;
	bool showLeftHandParticleGizmo_ = true;
	bool leftHandParticleEmitRequested_ = false;
	float leftHandWaterEffectRemaining_ = 0.0f;
	float leftHandWaterEmitAccumulator_ = 0.0f;
	float animationCrossFadeDuration_ = 0.25f;
	...
	bool showLegacySceneDebugWindows_ = false;
#endif
