...
ID3D12Resource* CreateTextureResource(ID3D12Device* device, const DirectX::TexMetadata& metadata) {
	...
	HRESULT hr = device->CreateCommittedResource(
		...
		D3D12_RESOURCE_STATE_COPY_DEST, //初回のResourceState。Textureは基本読むだけで
		nullptr,//Clear最適値。使わないのでnullput
		IID_PPV_ARGS(&resource));  //作成するResourceポインタへのチェック
	assert(SUCCEEDED(hr));
	return resource;
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	descriptorRange[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;//SUVを使う
	descriptorRange[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;//Offsetを自動計算

	rootParamenters[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;//DescriptorTableを使う
	rootParamenters[2].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;//PixelShaderを使う
	...
	*transformationMatrixDataSprite = MatrixMath::MakeIdentity4x4();
	...
	///////////////////////////////////////////////////////////////
		...
		ID3D12Resource* intermediateResoirce = UploadTextureData(textureResource, mipImages, device, commandList);
		...
		while (msg.message != WM_QUIT) {
			...
			if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
				...
				*wvpData = worldViewProjectionMatrix;
				...
				Matrix4x4 worldViewProjectionMatrixDataSprite = MatrixMath::Multiply(worldMatrixSprite, MatrixMath::Multiply(viewMatrixSprite, projectionMatrixSprite));
				*transformationMatrixDataSprite = worldViewProjectionMatrixDataSprite;
